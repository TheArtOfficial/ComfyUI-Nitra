...
export async function updateWorkflowInstallButton(options = {}) {
    ...
    try {
        ...
        console.error('Error updating workflow button state:', error);
    ...
    if (hfTokenContainer) {
        ...
        const prefixElement = document.getElementById('nitra-workflow-hf-token-prefix');
        if (prefixElement) {
            if (requiresHFToken) {
        ...
        const helpElement = document.getElementById('nitra-workflow-hf-token-help');
        if (helpElement) {
            if (requiresHFToken) {
