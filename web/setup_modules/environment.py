...
def sync_onnxruntime_version(config: ComfyUIConfig) -> None:
    ...
    run_command(config.venv_pip + ['uninstall', '-y', 'onnxruntime'], log_output=False)
    run_command(config.venv_pip + ['install', '--upgrade', '--no-warn-script-location', 'onnxruntime-gpu', 'onnx'], log_output=False)
...
def install_frontend_package(config: ComfyUIConfig) -> None:
    ...
    run_command(config.venv_pip + ['install', '-U', '--no-warn-script-location', 'comfyui_frontend_package'], log_output=False)
